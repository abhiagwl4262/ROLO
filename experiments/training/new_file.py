...
# Imports
import sys
sys.path.append("/home/abhishek/ROLO/ROLO-master/utils")
sys.path.append("/home/abhishek/ROLO/ROLO-master/update/src")
...
class ROLO_TF:
    ...
    rolo_weights_file = '/home/abhishek/ROLO/ROLO-master/ROLO_models/model_step6_exp1.ckpt' 
    ...
    def LSTM_single(self, name,  _X, _istate, _weights, _biases):
        ...
        _X = tf.reshape(_X, [self.batch_size, self.num_steps,  self.num_input]) # (num_steps*batch_size, num_input)
        print _X.get_shape() 
        
        # Split data beuse rnn cell needs a list of inputs for the RNN inner loop
        ...
        # _X = tf.split(_X, self.batch_size, 0 )
        # print _X.get_shape()
        
        # cell = tf.nn.rnn_cell.LSTMCell(self.num_input, state_is_tuple=True)
        cell = tf.nn.rnn_cell.LSTMCell(2*self.num_input, state_is_tuple=True)
        _istate = cell.zero_state(self.batch_size, tf.float32)
        state = _istate
        outputs, state = tf.nn.dynamic_rnn(cell, _X, initial_state=state)

        
        # print state.get_shape()
        # outputs, state = tf.nn.dynamic_rnn(cell,_X, state)
        tf.get_variable_scope().reuse_variables()
    ...
    def train_20(self):
        ...
        log_file = open("output/training-20-log.txt", "w") #open in append mode
        self.build_networks()
        ...
        self.optimizer = tf.train.GradientDescentOptimizer(learning_rate=self.learning_rate).minimize(self.accuracy) # Adam Optimizer
